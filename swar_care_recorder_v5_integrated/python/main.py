        ...
        <script>
            ...
            async function syncData() {
                ...
                if (tryHttp && !fetchInFlight) {
                    ...
                    try {
                        const res = await fetch('http://localhost:7654/status', {cache: 'no-store', signal: controller.signal});
                        if (res.ok) {
        ...
        <script>
            ...
            async function syncTerminal() {
                ...
                if (tryHttp && !fetchInFlight) {
                    ...
                    const timeoutId = setTimeout(() => controller.abort(), 300);
                    try {
                        const res = await fetch('http://localhost:7654/piezo_lines', {cache: 'no-store', signal: controller.signal});
                        if (res.ok) {
                            const data = await res.json();
            ...
            (function() {
                ...
                async function fetchAudioData() {
                    ...
                    if (tryHttp && !fetchInFlight) {
                        ...
                        const timeoutId = setTimeout(() => controller.abort(), 300);
                        try {
                            const res = await fetch('http://localhost:7654/audio_data', {cache: 'no-store', signal: controller.signal});
                            if (res.ok) {
                                const data = await res.json();

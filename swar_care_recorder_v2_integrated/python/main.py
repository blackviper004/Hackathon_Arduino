...
# ==========================================
# 🧠 TOP POLISHED AI ANOMALY MONITOR DECK
# ==========================================
st.subheader("🧠 Real-Time AI Diagnostic & Anomaly Center")

if os.path.exists(DATA_DIR):
    all_recordings = sorted([f.replace("_piezo.csv", "").replace("_audio.wav", "") for f in os.listdir(DATA_DIR)], reverse=True)
...
if unique_prefixes:
    ...
    # Styled Card Container for AI Results
    with st.container(border=True):
        col_ai1, col_ai2, col_ai3 = st.columns(3)
        
        status_color_text = ai_results["status"]
        with col_ai1:
            st.markdown(f"**Diagnostic Status**\n### {status_color_text}")
        with col_ai2:
            st.metric("Anomaly Score (0-1)", f"{ai_results['score']:.3f}")
            st.progress(min(1.0, ai_results['score']))
        with col_ai3:
            st.metric("Confidence Level", f"{ai_results['confidence']}%")
            st.progress(ai_results['confidence'] / 100.0)
    ...
    with st.container(border=True):
        st.info("💡 **AI Engine Standing By:** Record a session using the control deck below to trigger live background feature extraction and anomaly classification.")

...
class SwarCareEngine:
    ...
    def __init__(self):
        ...
        # Optimized Storage Targets (Only CSV for Piezo, Only temporary raw data for Audio)
        self.piezo_csv = None
    ...
    def start_recording(self):
        with self.state_lock:
            ...
            # Open real-time streaming file handles immediately
            ...
            with self.audio_file_lock:
                ...
                self.audio_raw_file = open(self.audio_tmp_path, "wb")
            ...
            # Purge any stale hardware frames from the ALSA system kernel pipe allocation
    ...
    def _safe_single_threaded_cleanup(self):
        ...
        with self.piezo_file_lock, self.audio_file_lock:
            # Calculate absolute matching limits based on identical Master Epoch Start Point
            ...
            # --- 1. CLOSE AND TRUNCATE PIEZO CSV ---
            if self.piezo_csv:
                self.piezo_csv.flush()
            ...
            # --- 2. COMPILE TRUNCATED NATIVE WAV FILE FROM RAW BUFFER ---
            if self.audio_raw_file:
            ...
            try:
                ...
                # Open final wave descriptor and apply strict hardware timeline parameters
                wav_file = wave.open(final_wav_path, "wb")
                wav_file.setnchannels(1)
                wav_file.setsampwidth(2)
                wav_file.setframerate(self.AUDIO_SAMPLE_RATE_HZ)
                
                # Extract exactly matching samples from start to drop trailing driver overrun bytes
                ...
                # Delete temporary intermediate scratchpad tracking file from disk

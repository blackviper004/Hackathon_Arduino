...
# analyze_recording_ai, and the plain attributes such as .state,
...
@app.get("/api/anomaly_analysis")
def anomaly_analysis():
    """Exposes the general (non-Veena) anomaly model too, in case the
    frontend wants it later — main.py never rendered this, so it's not part
    of the default UI, just kept available since engine.py already exposes it."""
    prefix = _latest_prefix()
    if not prefix:
        return {"available": False, "error": "No recording session found yet."}
    return backend.analyze_recording_ai(prefix)
